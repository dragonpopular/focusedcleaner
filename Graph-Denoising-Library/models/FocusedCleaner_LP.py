...
class LinkPrediction(Module):
    ...
    def adj_to_triple(self, adj, node_idx):
        ...
        link_idx = torch.tensor(np.asarray(np.triu_indices(len(adj_), k=1)).T)
...
class FocusedCleaner(Module):
    ...
    def forward(self, features, ori_adj, adj_clean, labels, idx_train, idx_val, idx_test, perturbations):
        ...
        for i in tqdm(range(perturbations), desc="Sanitizing"):
            ...
            LP_thresh = self.train_LinkPrediction(torch.cat((gcn_feature.clone().detach(), attr_dr), 1), modified_adj,
                                                  list(all_node_set))
            'victim node detection.'
            abnormal_node_idx = self.AbnormalDetection(torch.cat((gcn_feature.clone().detach(), attr_dr), 1),
                                                       modified_adj, LP_thresh)
        ...
        print('anomaly auc', np.round(self.gcn_acc[-1], 2))
        print('link pred auc', np.round(self.adversarial_detection_auc[-1], 2))
