...
from layers.DGMM_gcn import GCN
...
class FocusedCleaner(Module):
    ...
    def prox_1(self, softmax, ori_adj):
        pairwise_KL = (((softmax * torch.log(softmax)).sum(1)).reshape(-1, 1).repeat(1, self.nnodes) - (
                    softmax @ torch.log(softmax.T)))
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_val, idx_test, labels, labels_self_training, smoother,
                      normal_node_idx):
        hidden = features
        for ix, w in enumerate(self.weights):
            b = self.biases[ix] if self.with_bias else 0
    ...
    def forward(self, features, ori_adj, adj_clean, labels, idx_train, idx_val, idx_test, perturbations):
        ...
        for i in tqdm(range(perturbations), desc="Sanitizing"):
            ...
            'inner training.'
            gcn_feature, softmax, labels_self_training = self.inner_train(features, adj_norm, idx_train, idx_val,
                                                                          idx_test, labels)
            'get low-dimensional attributes.'
            attr_dr = torch.from_numpy(
                PCA(n_components=self.nclass, random_state=6).fit_transform(features.cpu().data.numpy())).to(
                self.device).float()
            ...
            p1 = np.nan_to_num(self.prox_1(softmax, modified_adj).cpu().data.numpy().reshape(-1, 1), nan=0., posinf=0.,
                               neginf=0.)
            p2 = np.nan_to_num(self.prox_2(softmax, modified_adj).cpu().data.numpy().reshape(-1, 1), nan=0., posinf=0.,
                               neginf=0.)
            js = np.nan_to_num(self.JSDiv(softmax, modified_adj).cpu().data.numpy(), nan=0., posinf=0., neginf=0.)
            p1a = np.nan_to_num(self.prox_1(attr_softmax, modified_adj).cpu().data.numpy().reshape(-1, 1), nan=0.,
                                posinf=0., neginf=0.)
            p2a = np.nan_to_num(self.prox_2(attr_softmax, modified_adj).cpu().data.numpy().reshape(-1, 1), nan=0.,
                                posinf=0., neginf=0.)
            ...
            lambda_cov = 0.
            dgmm = Model(n_cluster, dec_feats_torch.shape[1], 100, lambda_cov, 0., self.device).to(self.device)
            ...
            true_victim_nodes = list((
            set(np.where(np.triu(modified_adj.cpu().data.numpy()) != np.triu(np.array(adj_clean.todense())))[0]).union( \
            ...
            adj_grad = self.get_meta_grad(features, adj_norm, idx_train, idx_val, idx_test, labels,
                                          labels_self_training, feat_smoother, normal_node_idx)
